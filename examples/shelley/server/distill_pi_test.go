...
type refusingChat struct{ damodel.Chat }

func (r refusingChat) Invoke(context.Context, damodel.Request) (damodel.Response, error) {
	return damodel.Response{Message: dmessage.Message{Role: dmessage.RoleAssistant}}, nil
}

func (p *refusingModelProvider) GetChat(modelID string) (damodel.Chat, error) {
...
// TestPiDistillDoesNotSubstituteModelsOnRefusal verifies that compaction uses
// exactly the requested model and rolls back if it returns no summary.
func TestPiDistillDoesNotSubstituteModelsOnRefusal(t *testing.T) {
	t.Parallel()
	synctest.Test(t, func(t *testing.T) {
		h := NewTestHarness(t)
		defer stopActiveConversationLoops(h.server)
		// Force summarization of the older slice.
		h.server.piDistillKeepRecentTokens = 1
		// The requested model always returns empty content. The server must not
		// silently substitute its default model.
		...
		if !sawError {
			t.Fatal("expected compaction failure in the restored generation")
		}
		if sawSummary {
			t.Fatal("unexpected summary from a substituted model")
		}
	})
}
