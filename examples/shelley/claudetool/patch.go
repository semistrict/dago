...
func (p *PatchTool) NativeTool() dtool.Tool {
	description, schema := p.definition()
	return dtool.Func{
		Spec: dtool.Definition{Name: PatchName, Description: description, InputSchema: json.RawMessage(schema)},
...
func (p *PatchTool) patchRun(ctx context.Context, input *PatchInput) (patchExecution, error) {
	path := input.Path
	...
	if len(input.Patches) == 0 {
		return patchExecution{}, fmt.Errorf("no patches provided")
	}
	...
	case errors.Is(err, os.ErrNotExist):
		for _, patch := range input.Patches {
			...
			default:
				return patchExecution{}, fmt.Errorf("file %q does not exist", input.Path)
			}
		}
	case err != nil:
		return patchExecution{}, fmt.Errorf("failed to read file %q: %w", input.Path, err)
	}
	...
	for i, patch := range input.Patches {
		...
		if patch.ToClipboard != "" {
			if patch.Operation != "replace" {
				return patchExecution{}, fmt.Errorf("toClipboard (%s): can only be used with replace operation", patch.ToClipboard)
			}
			if patch.OldText == "" {
				return patchExecution{}, fmt.Errorf("toClipboard (%s): oldText cannot be empty when using toClipboard", patch.ToClipboard)
			}
		...
		if patch.FromClipboard != "" {
			...
			if !ok {
				return patchExecution{}, fmt.Errorf("fromClipboard (%s): no clipboard with that name", patch.FromClipboard)
			}
		...
		if patch.Reindent != nil {
			...
			if err != nil {
				return patchExecution{}, fmt.Errorf("reindent(%q -> %q): %w", patch.Reindent.Strip, patch.Reindent.Add, err)
			}
		...
		case "replace":
			if patch.OldText == "" {
				return patchExecution{}, fmt.Errorf("patch %d: oldText cannot be empty for %s operation", i, patch.Operation)
			}
			...
			continue
		default:
			return patchExecution{}, fmt.Errorf("unrecognized operation %q", patch.Operation)
		}
	}
	...
	if patchErr != nil {
		...
		return patchExecution{}, fmt.Errorf("%s", errorMsg)
	}

	patched, err := buf.Bytes()
	if err != nil {
		return patchExecution{}, err
	}
	if err := os.MkdirAll(filepath.Dir(input.Path), 0o700); err != nil {
		return patchExecution{}, fmt.Errorf("failed to create directory %q: %w", filepath.Dir(input.Path), err)
	}
	if err := os.WriteFile(input.Path, patched, 0o600); err != nil {
		return patchExecution{}, fmt.Errorf("failed to write patched contents to file %q: %w", input.Path, err)
	}
	...
	return patchExecution{Output: response.String(), Display: displayData}, nil
}
