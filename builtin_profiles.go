...
func builtinHarnessProfile(provider, model string) (Profile, bool) {
	...
	if provider == "anthropic" {
		var prompt string
		switch model {
		case "claude-haiku-4-5", "claude-sonnet-4-6":
			prompt = anthropicUniversalPrompt
		case "claude-opus-4-7":
			prompt = anthropicOpusPrompt
		}
		if prompt != "" {
			return Profile{Name: provider + ":" + model, SystemPromptSuffix: &prompt}, true
		}
	}
	return builtinEngineeringHarnessProfile(provider, model)
}
