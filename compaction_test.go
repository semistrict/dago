...
func TestCompactConversationKeepsToolExchangeIntact(t *testing.T) {
	...
	formatted := 0
	result, err := CompactConversation(t.Context(), model, messages,
		WithCompactKeepTokens(25), WithCompactSystemPrompt("system"), WithCompactPrompt("Summarize"),
		WithCompactInstructions("retain paths"), WithCompactReasoning(damodel.Reasoning{Effort: "off"}),
		WithCompactHistoryFormatter(func(older []damessage.Message) (string, error) {
			formatted = len(older)
...
func TestCompactConversationSkipsModelWhenHistoryFits(t *testing.T) {
	...
	result, err := CompactConversation(t.Context(), model, []damessage.Message{damessage.Human("hello")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompactConversationHonorsApplicationRecordBoundaries(t *testing.T) {
	...
	result, err := CompactConversation(t.Context(), model, messages,
		WithCompactKeepTokens(25), WithCompactCutoffs(0, 3, 5),
		WithCompactHistoryFormatter(func(older []damessage.Message) (string, error) {
