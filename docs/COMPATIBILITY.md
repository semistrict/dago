...
| Persistent JavaScript interpreter and programmatic tool calling | verified | normal Go builds use the pinned quickjs-rs WASM guest under Wazero's portable interpreter; TinyGo builds reject explicit enablement and Shelley omits `js_eval`; full applicable upstream suite map in `docs/QUICKJS_TEST_PORT.md`; Promise settlement, typed PTC, interruption, WAFL dirty-page checkpoints, memory/SQLite/PostgreSQL restore, subagent dispatch, opt-in transparent tool lifecycle projection, and browser execution tests |
...
| Direct Anthropic Messages API | verified | `daproviders/anthropic` covers native SSE streaming, structured output, adaptive reasoning effort, cache breakpoints and cache usage, citations, media, custom/parallel tools, hosted web search/fetch, code execution, computer use, memory, tool search, remote MCP, context management, task budgets, user profiles, automatic current beta headers, exact hosted-block replay, bounded responses, cancellation, and transient-only retries; raw hosted-tool and top-level parameter boundaries preserve additive API features |
| Claude CLI model provider | verified | `daproviders/claudeagent` uses a persistent print-mode bidirectional stream-JSON process with incremental text/reasoning chunks, cache read/write accounting, the ambient local subscription login, exact caller system prompt, an empty workspace, disabled local setting sources and browser integration, and only the built-in `Skill` tool enabled; effective skills live in a private bounded session plugin with shell execution disabled, and oversized MCP descriptions become generated skills with short tool-loading instructions; request tools are schema-preserved behind an authenticated ephemeral loopback MCP server, outer tool results fulfill pending calls in the same process, internal skill calls continue inside the CLI, and native JSONL plus `--resume` is used only to reconstruct a replacement process; installed-binary coverage verifies hostile local memory/hook suppression, full generated-skill loading, shortened long-tool forwarding, separate parallel-call envelopes, repeated tool names, successful and failed results, replay-envelope filtering, a second tool round, two forced restarts, and project/plugin-entry cleanup, while separately gated coverage verifies real logged-in incremental output and restart reconstruction |
...
| Built-in web tools and SSRF boundary | verified | opt-in `daweb` HTTP request, HTML-to-Markdown page fetch, and Tavily search tools; public-address validation, DNS pinning, per-hop redirect checks, proxy bypass, cross-origin credential/body containment, cancellation, and finite request/response/render/redirect/time limits; dacode always binds `fetch_url`, enables OpenAI's hosted `web_search` by default, prefers provider-hosted search for any resolved model capability (including Anthropic integrations), and removes a same-named local fallback; otherwise a stored-over-environment Tavily service credential supplies the approval-gated fallback; adversarial tests use no live network |
...
| Deterministic JavaScript workflows | verified | runtime module metadata export; bounded parallel barriers and item pipelines; structured agent results through a required synthetic result tool with schema failures returned to the same worker and three bounded correction attempts; phases and logs; cancellation; token, concurrency, lifetime, and collection caps; one-level nesting; background management tools; persisted scripts, journals, agent records, and output; exact-prefix same-session replay; local Git worktree isolation with committed-branch retention and dirty-checkout preservation; terminal live workflow panel with running-agent elapsed time and streamed token estimates, saved-script launch, local Git isolation lifecycle, refresh, selection, and cancellation with Playwright coverage |
...
| Agent Client Protocol v1 server | verified | initialize/auth/new/load/config/prompt/cancel/close lifecycle; factory-backed dynamic model selection with advertised-value validation, history-preserving rebuilds, durable selection restore, and replacement cleanup; versioned model discovery derived from the session configuration; replay-marked durable transcripts; image, audio, and embedded content conversion; text, reasoning, tool, progress, and plan projection; approve/reject permission resume; per-session stdio/HTTP/SSE MCP discovery and invocation; versioned dago workflow update/list/cancel extensions with full-snapshot resynchronization; stop reasons; and transport tests in `daacp` and `dacode`; see `docs/ACP.md` |
...
- The ACP adapter implements the stable version 1 baseline, session close, dynamic
  ...
  free-form responses. Background workflow observability is a dago-specific, versioned
  ACP extension documented in `docs/ACP.md`; clients resynchronize from full snapshots
  rather than depending on lossless notification delivery.
