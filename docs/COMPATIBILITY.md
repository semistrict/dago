...
| Claude CLI model provider | verified | `daproviders/claudeagent` uses a persistent print-mode bidirectional stream-JSON process with incremental text/reasoning chunks, the ambient local subscription login, exact caller system prompt, an empty workspace, and disabled local setting sources, built-in tools, skills, browser integration, suggestions, and hooks/customization inputs; request tools are schema-preserved behind an authenticated ephemeral loopback MCP server, outer tool results fulfill pending calls in the same process, and native JSONL plus `--resume` is used only to reconstruct a replacement process; installed-binary coverage verifies hostile local memory/hook suppression, separate parallel-call envelopes, repeated tool names, successful and failed results, replay-envelope filtering, a second tool round, two forced restarts, and project-entry cleanup, while separately gated coverage verifies real logged-in incremental output and restart reconstruction |
