...
func TestStateBackendRequiresBindingAndEmitsPlainDataDelta(t *testing.T) {
	...
	ctx, err := value.BindRuntime(context.Background(), dastate.Values{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLocalShellUsesDynamicRootForEveryOperation(t *testing.T) {
	first := t.TempDir()
	second := t.TempDir()
	if err := os.WriteFile(filepath.Join(second, "visible.txt"), []byte("second root"), 0o600); err != nil {
		t.Fatal(err)
	}
	current := first
	shell, err := NewLocalShell(LocalShellOptions{Filesystem: FilesystemOptions{
		Root: first, GetRoot: func() string { return current }, AllowHostPaths: true,
	}})
	if err != nil {
		t.Fatal(err)
	}
	current = second
	listing, err := shell.List(t.Context(), ".")
	if err != nil || len(listing.Entries) != 1 || filepath.Base(listing.Entries[0].Path) != "visible.txt" {
		t.Fatalf("dynamic list = %#v, %v", listing, err)
	}
	result, err := shell.Execute(t.Context(), "pwd", time.Second)
	resolvedSecond, resolveErr := filepath.EvalSymlinks(second)
	if err != nil || resolveErr != nil || strings.TrimSpace(result.Output) != resolvedSecond {
		t.Fatalf("dynamic execute = %#v, %v", result, err)
	}
}
