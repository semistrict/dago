...
func TestDurableStoreOperationsAndRestart(t *testing.T) {
	...
	namespace := store.Namespace{"users", "one"}
	...
	defer second.Close()
	items, err := second.Search(context.Background(), store.SearchOptions{Prefix: store.Namespace{"users"}, Query: "delta"})
	if err != nil || len(items) != 1 {
		t.Fatalf("items = %#v, %v", items, err)
	}
	namespaces, err := second.ListNamespaces(context.Background(), store.Namespace{"users"})
	if err != nil || !reflect.DeepEqual(namespaces, []store.Namespace{namespace}) {
...
func TestBatchIsAtomicAndOrdered(t *testing.T) {
	...
	defer storage.Close()
	namespace := store.Namespace{"batch"}
	results, err := storage.Batch(context.Background(), []store.Operation{
