...
func (predictable *Predictable) Profile() model.Profile { return predictable.profile }

func (predictable *Predictable) Invoke(ctx context.Context, request model.Request) (model.Response, error) {
	...
	input := ""
	if ok && last.Role == message.RoleHuman {
	...
	if reasoning, found := strings.CutPrefix(input, "think: "); found {
		...
		response.Message.Content = append([]message.ContentBlock{{
			Type: message.BlockReasoning, Reasoning: reasoning,
	...
	if value, found := strings.CutPrefix(input, "structured: "); found {
		...
		if !json.Valid(structured) {
			return model.Response{}, fmt.Errorf("predictable model: structured output is not valid JSON")
		}
	...
	if text, found := strings.CutPrefix(input, "error: "); found {
		return model.Response{}, fmt.Errorf("predictable error: %s", text)
	}
	return predictable.textResponse(id, predictable.defaultResponse, request), nil
}

func (predictable *Predictable) Stream(ctx context.Context, request model.Request) (model.Stream, error) {
	response, err := predictable.Invoke(ctx, request)
	if err != nil {
		return nil, err
	}
	return &predictableStream{chunk: model.Chunk{
...
// tokenizer fidelity.
func (predictable *Predictable) CountTokens(ctx context.Context, messages []message.Message) (int, error) {
...
func (predictable *Predictable) RecentRequests() []model.Request {
	predictable.mu.Lock()
	defer predictable.mu.Unlock()
	result := make([]model.Request, len(predictable.recentRequests))
...
func (predictable *Predictable) LastRequest() (model.Request, bool) {
	predictable.mu.Lock()
	defer predictable.mu.Unlock()
...
func (predictable *Predictable) record(request model.Request) (uint64, time.Duration) {
	predictable.mu.Lock()
	defer predictable.mu.Unlock()
	predictable.nextID++
...
func (predictable *Predictable) textResponse(id uint64, text string, request model.Request) model.Response {
	result := message.Assistant(text)
	result.ID = fmt.Sprintf("predictable-%d", id)
	result.Usage = predictableUsage(request, result)
	return model.Response{Message: result}
}

func (predictable *Predictable) toolResponse(id uint64, name string, arguments json.RawMessage, request model.Request) model.Response {
	result := message.Assistant("I'll call " + name + ".")
	result.ID = fmt.Sprintf("predictable-%d", id)
	result.ToolCalls = []message.ToolCall{{
...
func predictableUsage(request model.Request, response message.Message) *message.Usage {
	inputCharacters := 0
...
func messageCharacters(item message.Message) int {
	characters := 0
...
func lastConversationMessage(messages []message.Message) (message.Message, bool) {
	for index := len(messages) - 1; index >= 0; index-- {
		if messages[index].Role != message.RoleSystem && messages[index].Role != message.RoleRemove {
...
func (stream *predictableStream) Next(ctx context.Context) (model.Chunk, error) {
	stream.mu.Lock()
