...
func TestPredictableTextAndToolPatterns(t *testing.T) {
	...
	response, err := predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human("echo: hello world")},
	})
	...
	response, err = predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human(`tool: lookup {"query":"dago"}`)},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	response, err = predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Tool("predictable-tool-2", "result")},
	})
...
func TestPredictableReasoningStructuredAndDefault(t *testing.T) {
	...
	response, err := predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human("think: inspect first")},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	response, err = predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human(`structured: {"answer":42}`)},
	})
	...
	response, err = predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human("unknown")},
	})
...
func TestPredictableErrorsAndCancellation(t *testing.T) {
	predictable := NewPredictable(PredictableOptions{})
	_, err := predictable.Invoke(context.Background(), model.Request{
		Messages: []message.Message{message.Human("error: expected")},
	})
	...
	_, err = predictable.Invoke(ctx, model.Request{Messages: []message.Message{message.Human("hello")}})
...
func TestPredictableHistoryIsBoundedAndIsolated(t *testing.T) {
	...
	request := model.Request{Messages: []message.Message{message.Human("echo: one")}}
	for _, text := range []string{"echo: one", "echo: two", "echo: three"} {
		request.Messages[0] = message.Human(text)
...
func TestPredictableStreamAndTokenCount(t *testing.T) {
	predictable := NewPredictable(PredictableOptions{})
	stream, err := predictable.Stream(context.Background(), model.Request{
		Messages: []message.Message{message.Human("echo: streamed")},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	tokens, err := predictable.CountTokens(context.Background(), []message.Message{message.Human("12345678")})
	if err != nil || tokens != 2 {
...
func TestPredictableRejectsMalformedCommands(t *testing.T) {
	...
	for _, input := range []string{"tool: missing-json", "tool: lookup {", "structured: {", "delay: later", "delay: -1s"} {
		t.Run(input, func(t *testing.T) {
			_, err := predictable.Invoke(context.Background(), model.Request{
				Messages: []message.Message{message.Human(input)},
			})
			if err == nil {
