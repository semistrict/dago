...
func nemotronEntityResolutionGuard() agent.Middleware {
	...
	return agent.Middleware{
		...
		Fields: map[string]agent.StateField{
			preKey:   {Kind: agent.FieldLast, Contract: "dago.nemotron.entity.pre.v1", Private: true, Clone: nemotronIdentityClone},
			firedKey: {Kind: agent.FieldLast, Contract: "dago.nemotron.entity.final.v1", Private: true, Clone: nemotronIdentityClone},
		},
		BeforeModel: func(_ context.Context, values state.Values, _ agent.Runtime) (state.Values, error) {
			...
			if err != nil || len(messages) == 0 || messages[len(messages)-1].Role != message.RoleTool {
				return nil, err
			}
		...
		AfterAgent: func(_ context.Context, values state.Values, _ agent.Runtime) (state.Values, error) {
			...
			update := nemotronNudge(text, nemotronEntitySource, agent.JumpUpdate("model"))
...
func missingNemotronResolutions(messages []message.Message, userText, finalText string) []nemotronResolution {
...
func currentNemotronEntityIDs(messages []message.Message) map[string]int64 {
...
func nemotronRelationBindings(messages []message.Message) map[nemotronRelationKey]int64 {
...
func resolvedNemotronEntities(messages []message.Message) map[nemotronEntityKey]bool {
...
func missingNemotronCurrentBranch(messages []message.Message, userText, finalText string) []nemotronResolution {
...
func missingNemotronDisplay(messages []message.Message, userText, finalText string) []nemotronResolution {
	resolved := resolvedNemotronEntities(messages)
	visible := visibleNemotronIDs(finalText)
	userLower := strings.ToLower(userText)
	var missing []nemotronResolution
...
func nemotronArguments(call message.ToolCall) map[string]any {
