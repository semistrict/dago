...
// Config is the JSON/YAML-safe subset of a runtime harness profile.
...
type Config struct {
	...
	GeneralPurposeSubagent   *dago.GeneralPurposeSubagentProfile
}
...
var generalPurposeProfileKeys = map[string]bool{
	"mode": true, "description": true, "system_prompt": true,
}
...
func FromMap(data map[string]any) (Config, error) {
	for key := range data {
		if !harnessProfileConfigKeys[key] {
			return Config{}, fmt.Errorf("unknown harness profile config key %q", key)
		}
	}
	var result Config
	var err error
...
func (config Config) ToProfile() (dago.Profile, error) {
	if err := config.Validate(); err != nil {
		return dago.Profile{}, err
	}
	return dago.Profile{
		BaseSystemPrompt:   configCloneStringPointer(config.BaseSystemPrompt),
...
func FromProfile(profile dago.Profile) (Config, error) {
	if len(profile.Middleware) > 0 || profile.SystemPrompt != "" {
		return Config{}, fmt.Errorf("runtime middleware and additive system prompts cannot be represented in a harness profile config")
	}
	result := Config{
...
func generalPurposeProfileMap(profile dago.GeneralPurposeSubagentProfile) map[string]any {
	result := map[string]any{}
	if profile.Mode != "" {
		result["mode"] = string(profile.Mode)
	}
...
func generalPurposeProfileFromValue(value any) (*dago.GeneralPurposeSubagentProfile, error) {
	...
	result := &dago.GeneralPurposeSubagentProfile{}
	var err error
	if value, exists := data["mode"]; exists && value != nil {
		mode, ok := value.(string)
		if !ok {
			return nil, fmt.Errorf("general_purpose_subagent.mode must be a string, got %T", value)
		}
		result.Mode = dago.GeneralPurposeSubagentMode(mode)
		if result.Mode != dago.GeneralPurposeSubagentEnabled && result.Mode != dago.GeneralPurposeSubagentDisabled {
			return nil, fmt.Errorf("general_purpose_subagent.mode must be %q or %q", dago.GeneralPurposeSubagentEnabled, dago.GeneralPurposeSubagentDisabled)
		}
	}
...
func configCloneGeneralPurposeProfile(value *dago.GeneralPurposeSubagentProfile) *dago.GeneralPurposeSubagentProfile {
	if value == nil {
		return nil
	}
	return &dago.GeneralPurposeSubagentProfile{
		Mode: value.Mode, Description: configCloneStringPointer(value.Description),
