...
func TestSaverPutGetListAndMetadataFilter(t *testing.T) {
	...
	c1, err := saver.Put(ctx, root, cp1, checkpoint.Metadata{
		Source: "input", Step: 0, Extra: map[string]any{"tenant": "a"},
	}, nil)
	if err != nil {
		t.Fatalf("Put(cp1) error = %v", err)
	}
	if err := saver.PutWrites(ctx, c1, "task", "ignored", []checkpoint.ChannelWrite{
	...
	c2, err := saver.Put(ctx, c1, cp2, checkpoint.Metadata{Source: "loop", Step: 1}, nil)
	if err != nil {
	...
	rows, err := saver.List(ctx, &root, checkpoint.ListOptions{
...
func TestSaverWriteConflictBehavior(t *testing.T) {
	...
	if _, err := saver.Put(ctx, checkpoint.Config{ThreadID: "thread"}, testCheckpoint("cp", nil), checkpoint.Metadata{}, nil); err != nil {
		t.Fatalf("Put() error = %v", err)
	}
	if err := saver.PutWrites(ctx, config, "task", "", []checkpoint.ChannelWrite{
		{Channel: "value", Value: "first"}, {Channel: checkpoint.ChannelError, Value: "old"},
	}); err != nil {
	...
	if err := saver.PutWrites(ctx, config, "task", "", []checkpoint.ChannelWrite{
		{Channel: "value", Value: "ignored"}, {Channel: checkpoint.ChannelError, Value: "also ignored"},
	}); err != nil {
		t.Fatalf("PutWrites(second) error = %v", err)
	}
	// An all-special batch uses replacement.
	if err := saver.PutWrites(ctx, config, "task", "", []checkpoint.ChannelWrite{
		{Channel: checkpoint.ChannelError, Value: "new"},
	}); err != nil {
	...
	if values["value"] != "first" || values[checkpoint.ChannelError] != "new" {
...
func TestSaverDeltaHistoryUsesSeedAncestorAndExcludesTarget(t *testing.T) {
	...
	c0, _ := saver.Put(ctx, root, cp0, checkpoint.Metadata{}, nil)
	_ = saver.PutWrites(ctx, c0, "task-0", "", []checkpoint.ChannelWrite{{Channel: "items", Value: []any{"subsumed"}}})
	cp1 := testCheckpoint("cp1", map[string]any{"items": checkpoint.DeltaSnapshot{Value: []any{"seed"}}})
	c1, _ := saver.Put(ctx, c0, cp1, checkpoint.Metadata{}, nil)
	_ = saver.PutWrites(ctx, c1, "task-a", "", []checkpoint.ChannelWrite{{Channel: "items", Value: []any{"first"}}})
	_ = saver.PutWrites(ctx, c1, "task-b", "", []checkpoint.ChannelWrite{{Channel: "items", Value: []any{"second"}}})
	cp2 := testCheckpoint("cp2", map[string]any{})
	c2, _ := saver.Put(ctx, c1, cp2, checkpoint.Metadata{}, nil)
	_ = saver.PutWrites(ctx, c2, "target", "", []checkpoint.ChannelWrite{{Channel: "items", Value: []any{"pending"}}})
	...
	if _, ok := history.Seed.(checkpoint.DeltaSnapshot); !ok {
...
func TestSaverCopyDeleteAndPrune(t *testing.T) {
	ctx := context.Background()
	saver := openTestSaver(t)
	root := checkpoint.Config{ThreadID: "source"}
	c1, _ := saver.Put(ctx, root, testCheckpoint("cp1", nil), checkpoint.Metadata{}, nil)
	_, _ = saver.Put(ctx, c1, testCheckpoint("cp2", nil), checkpoint.Metadata{}, nil)
	// Add an abandoned fork that keep_latest may safely remove.
	_, _ = saver.Put(ctx, c1, testCheckpoint("branch", nil), checkpoint.Metadata{}, nil)
	if err := saver.Prune(ctx, []string{"source"}, checkpoint.PruneKeepLatest); err != nil {
	...
	if tuple, err := saver.GetTuple(ctx, checkpoint.Config{ThreadID: "target"}); err != nil || tuple == nil {
		t.Fatalf("target GetTuple() = %+v, %v", tuple, err)
	}
	if tuple, err := saver.GetTuple(ctx, checkpoint.Config{ThreadID: "source"}); err != nil || tuple != nil {
...
func TestSaverRejectsUnsafePayloadWithCheckpointContext(t *testing.T) {
	...
	_, err := saver.Put(context.Background(), checkpoint.Config{ThreadID: "thread"}, checkpointValue, checkpoint.Metadata{}, nil)
...
func TestReadsAndContinuesPinnedPythonSafeFixture(t *testing.T) {
	...
	defer saver.Close()
	ctx := context.Background()
	loaded, err := saver.GetTuple(ctx, checkpoint.Config{ThreadID: "python-safe"})
	...
	parsed, ok := messages[0].(message.Message)
	if !ok || parsed.TextContent() != "hello from Python" {
		t.Fatalf("message = %#v", messages[0])
	}
	if _, ok := loaded.Checkpoint.ChannelValues["delta"].(checkpoint.DeltaSnapshot); !ok {
	...
	child, err := saver.Put(ctx, loaded.Config, continued, checkpoint.Metadata{Source: "loop", Step: 1}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func testCheckpoint(id string, values map[string]any) checkpoint.Checkpoint {
	if values == nil {
		values = map[string]any{}
	}
	return checkpoint.Checkpoint{
		Version: checkpoint.LatestVersion, ID: id, Timestamp: id,
