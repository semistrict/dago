...
func TestSaverIntegration(t *testing.T) {
	...
	value := checkpoint.Checkpoint{
		Version: checkpoint.LatestVersion, ID: "00000000-0000-6000-8000-000000000001", Timestamp: "2026-01-01T00:00:00Z",
		ChannelValues:   map[string]any{"inline": "value", "delta": checkpoint.DeltaSnapshot{Value: []any{"seed"}}},
	...
	config, err := saver.Put(ctx, root, value, checkpoint.Metadata{Source: "input", Step: 0}, value.ChannelVersions)
	if err != nil {
		t.Fatal(err)
	}
	if err := saver.PutWrites(ctx, config, "task", "path", []checkpoint.ChannelWrite{{Channel: "delta", Value: []any{"next"}}}); err != nil {
		t.Fatal(err)
	}
	...
	if _, ok := tuple.Checkpoint.ChannelValues["delta"].(checkpoint.DeltaSnapshot); !ok {
...
func TestReadsAndContinuesPinnedPythonSafeFixture(t *testing.T) {
	...
	defer saver.Close()
	ctx := context.Background()
	tuple, err := saver.GetTuple(ctx, checkpoint.Config{ThreadID: "python-safe-postgres"})
	if err != nil || tuple == nil {
		t.Fatalf("GetTuple = %#v, %v", tuple, err)
	}
	...
	if _, ok := messages[0].(message.Message); !ok {
	...
	if _, err := saver.Put(ctx, tuple.Config, continued, checkpoint.Metadata{Source: "loop", Step: 1}, nil); err != nil {
		t.Fatal(err)
	}
}
