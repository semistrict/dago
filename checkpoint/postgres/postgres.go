...
func (saver *Saver) Put(
	...
	if err != nil {
		return checkpoint.Config{}, fmt.Errorf("prepare PostgreSQL checkpoint %q: %w", value.ID, err)
	}
...
ON CONFLICT (thread_id, checkpoint_ns, channel, version) DO NOTHING`,
			...
			return checkpoint.Config{}, fmt.Errorf("put PostgreSQL checkpoint blob %q: %w", channel, err)
		}
	}
	if _, err := transaction.ExecContext(ctx, `
...
DO UPDATE SET checkpoint = EXCLUDED.checkpoint, metadata = EXCLUDED.metadata`,
		...
		return checkpoint.Config{}, fmt.Errorf("put PostgreSQL checkpoint %q: %w", value.ID, err)
	}
	if err := transaction.Commit(); err != nil {
		return checkpoint.Config{}, err
	}
	return checkpoint.Config{ThreadID: config.ThreadID, Namespace: config.Namespace, CheckpointID: value.ID}, nil
}

func (saver *Saver) splitCheckpoint(value checkpoint.Checkpoint) (checkpoint.Checkpoint, map[string]serde.Typed, error) {
	inline := value
	...
	for channel, item := range value.ChannelValues {
		if snapshot, ok := item.(checkpoint.DeltaSnapshot); ok {
			encoded, err := saver.codec.Encode(snapshot)
			if err != nil {
				return checkpoint.Checkpoint{}, nil, err
			}
...
FROM checkpoints WHERE thread_id = $1 AND checkpoint_ns = $2`
	...
	actual := checkpoint.Config{ThreadID: config.ThreadID, Namespace: config.Namespace, CheckpointID: id}
	writes, err := saver.pendingWrites(ctx, actual)
	if err != nil {
		return nil, err
	}
	tuple := &checkpoint.Tuple{Config: actual, Checkpoint: value, Metadata: metadata, PendingWrites: writes}
	if parent.Valid && parent.String != "" {
		parentConfig := checkpoint.Config{ThreadID: config.ThreadID, Namespace: config.Namespace, CheckpointID: parent.String}
		tuple.Parent = &parentConfig
	}
	return tuple, nil
}

func (saver *Saver) loadBlobs(ctx context.Context, threadID, namespace string, value *checkpoint.Checkpoint) error {
...
func (saver *Saver) List(ctx context.Context, config *checkpoint.Config, options checkpoint.ListOptions) ([]checkpoint.Tuple, error) {
	if err := saver.Setup(ctx); err != nil {
		return nil, err
	}
	...
	var configs []checkpoint.Config
	for rows.Next() {
		var item checkpoint.Config
...
func (saver *Saver) GetDeltaChannelHistory(ctx context.Context, config checkpoint.Config, channels []string) (map[string]checkpoint.DeltaHistory, error) {
...
func unmarshalCheckpoint(data []byte) (checkpoint.Checkpoint, error) {
	var value checkpoint.Checkpoint
