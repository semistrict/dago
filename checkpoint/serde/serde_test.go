...
func TestSafeCheckpointRoundTrip(t *testing.T) {
	...
	call := message.Message{
		ID: "assistant", Role: message.RoleAssistant,
		ToolCalls: []message.ToolCall{{
			ID: "call", Name: "lookup", Arguments: json.RawMessage(`{"key":"x"}`),
		}},
	}
	input := checkpoint.Checkpoint{
		Version: checkpoint.LatestVersion, ID: "cp", Timestamp: "time",
		ChannelValues: map[string]any{
			"messages":  checkpoint.DeltaSnapshot{Value: []message.Message{call}},
			"overwrite": state.Overwrite{Value: map[string]any{"a": int64(1)}},
		},
	...
	decoded, ok := decodedAny.(checkpoint.Checkpoint)
	if !ok {
	...
	snapshot, ok := decoded.ChannelValues["messages"].(checkpoint.DeltaSnapshot)
	if !ok {
	...
	decodedMessage, ok := messages[0].(message.Message)
	if !ok || decodedMessage.ToolCalls[0].Name != "lookup" {
		t.Fatalf("decoded message = %#v", messages[0])
	}
	if _, ok := decoded.ChannelValues["overwrite"].(state.Overwrite); !ok {
...
func TestSafeMetadataRoundTrip(t *testing.T) {
	...
	input := checkpoint.Metadata{
		...
		CountersSinceDeltaSnapshot: map[string]checkpoint.DeltaCounter{
