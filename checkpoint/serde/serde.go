...
func checkpointToWire(value checkpoint.Checkpoint, depth, maxDepth int) (map[string]any, error) {
...
func metadataToWire(value checkpoint.Metadata, depth, maxDepth int) (map[string]any, error) {
...
func fromWire(value any) (any, error) {
	...
	case extension:
		...
		return checkpoint.DeltaSnapshot{Value: decoded}, nil
	case []any:
	...
	case map[string]any:
		...
		case "dago.overwrite.v1":
			return state.Overwrite{Value: result["value"]}, nil
...
func wireToCheckpoint(record map[string]any) (checkpoint.Checkpoint, error) {
	...
	if seen, ok := record["versions_seen"].(map[string]any); ok {
		for node, raw := range seen {
			...
			if !ok {
				return checkpoint.Checkpoint{}, fmt.Errorf("%w: versions_seen.%s", ErrMalformedPayload, node)
			}
...
func wireToMetadata(record map[string]any) (checkpoint.Metadata, error) {
	data := checkpoint.Metadata{
		...
		CountersSinceDeltaSnapshot: map[string]checkpoint.DeltaCounter{}, Extra: map[string]any{},
	}
	...
	if counters, ok := record["counters_since_delta_snapshot"].(map[string]any); ok {
		for key, raw := range counters {
			items, ok := raw.([]any)
			if !ok || len(items) != 2 {
				return checkpoint.Metadata{}, fmt.Errorf("%w: delta counter %q", ErrMalformedPayload, key)
			}
			data.CountersSinceDeltaSnapshot[key] = checkpoint.DeltaCounter{
