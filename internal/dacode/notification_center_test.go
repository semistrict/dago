...
func TestNotificationCenterNavigatesDetailsAndActions(t *testing.T) {
	...
	if request, close := center.handleKey(tea.KeyMsg{Type: tea.KeyDown}); request != nil || close || center.selected != 1 {
		t.Fatalf("down = %#v, %v, selected %d", request, close, center.selected)
	}
	center.handleKey(tea.KeyMsg{Type: tea.KeyEnter})
	if !center.detail || center.action != 0 {
		t.Fatalf("detail = %v action = %d", center.detail, center.action)
	}
	center.handleKey(tea.KeyMsg{Type: tea.KeyDown})
	request, close := center.handleKey(tea.KeyMsg{Type: tea.KeyEnter})
	if request == nil || request.Key != "tool" || request.ID != notificationSuppress || close {
		t.Fatalf("request = %#v close = %v", request, close)
	}
	center.handleKey(tea.KeyMsg{Type: tea.KeyEscape})
	if center.detail {
		t.Fatal("escape did not return to list")
	}
	if request, close := center.handleKey(tea.KeyMsg{Type: tea.KeyEscape}); request != nil || !close {
...
func TestNotificationSettingsToggleAndRollback(t *testing.T) {
	...
	settings.handleKey(tea.KeyMsg{Type: tea.KeyDown})
	key, enabled, changed, close := settings.handleKey(tea.KeyMsg{Type: tea.KeyEnter})
	...
	if _, _, _, close := settings.handleKey(tea.KeyMsg{Type: tea.KeyEscape}); !close {
