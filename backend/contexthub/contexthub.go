...
func (remote *Backend) List(ctx context.Context, directory string) (backend.ListResult, error) {
	...
	entries := map[string]backend.FileInfo{}
	for name, content := range remote.files {
		...
		if rest != "" {
			entries[entryName] = backend.FileInfo{Path: "/" + entryName, IsDir: true}
		} else {
			entries[entryName] = backend.FileInfo{Path: "/" + entryName, Size: int64(len(content))}
		}
	}
	result := backend.ListResult{Entries: make([]backend.FileInfo, 0, len(entries))}
...
func (remote *Backend) Write(ctx context.Context, filePath, content string) (backend.WriteResult, error) {
	...
	if name == "" {
		return backend.WriteResult{}, fmt.Errorf("write virtual root is not allowed")
	}
	remote.mu.Lock()
	defer remote.mu.Unlock()
	if err := remote.loadLocked(ctx); err != nil {
	...
	return backend.WriteResult{Path: virtual}, nil
}
...
func (remote *Backend) Edit(ctx context.Context, filePath, old, replacement string, replaceAll bool) (backend.EditResult, error) {
	if old == "" || old == replacement {
		return backend.EditResult{}, fmt.Errorf("edit: old string must be non-empty and differ from replacement")
	}
	virtual, name, err := cleanPath(filePath)
	if err != nil {
...
func (remote *Backend) Delete(ctx context.Context, filePath string) (backend.DeleteResult, error) {
	...
	if name == "" {
		return backend.DeleteResult{}, fmt.Errorf("delete virtual root is not allowed")
	}
	remote.mu.Lock()
	defer remote.mu.Unlock()
	if err := remote.loadLocked(ctx); err != nil {
	...
	if len(changes) == 0 {
		return backend.DeleteResult{}, fmt.Errorf("path %q: file not found", virtual)
	}
	if err := remote.commitLocked(ctx, changes); err != nil {
		return backend.DeleteResult{}, err
	}
	return backend.DeleteResult{Path: virtual}, nil
}

func (remote *Backend) Glob(ctx context.Context, pattern, _ string) (backend.GlobResult, error) {
	...
	for name, content := range remote.files {
		if matcher.MatchString(name) || matcher.MatchString("/"+name) {
			result.Matches = append(result.Matches, backend.FileInfo{Path: "/" + name, Size: int64(len(content))})
		}
	}
...
func (remote *Backend) Grep(ctx context.Context, pattern string, options backend.GrepOptions) (backend.GrepResult, error) {
	...
	if err != nil {
		return backend.GrepResult{}, fmt.Errorf("invalid regex pattern: %w", err)
	}
	...
	for _, name := range names {
		...
		for index, line := range splitContentLines(remote.files[name]) {
			...
			result.Matches = append(result.Matches, backend.GrepMatch{Path: "/" + name, Line: index + 1, Text: line})
		}
	}
	return result, nil
}
...
func (remote *Backend) Upload(ctx context.Context, uploads []backend.Upload) []backend.UploadResult {
	result := make([]backend.UploadResult, len(uploads))
	changes := map[string]*Entry{}
...
func (remote *Backend) Download(ctx context.Context, paths []string) []backend.DownloadResult {
	result := make([]backend.DownloadResult, len(paths))
	remote.mu.Lock()
	err := remote.loadLocked(ctx)
